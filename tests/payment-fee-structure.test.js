...
describe('Payment Fee Structure Tests', () => {
  ...
  describe('Payment Model Fee Calculations', () => {
    it('should calculate correct fees for $100 service amount', async () => {
      ...
      expect(payment.amount).to.equal(10000); // Service amount
      expect(payment.applicationFeeAmount).to.equal(1500); // $15.00 platform fee
      expect(payment.providerTaxAmount).to.equal(1495); // $14.95 tax
    ...
    it('should calculate correct fees for $50 service amount', async () => {
      ...
      expect(payment.amount).to.equal(5000);
      expect(payment.applicationFeeAmount).to.equal(1000); // $10.00 platform fee
      expect(payment.providerTaxAmount).to.equal(780); // $7.80 tax
      expect(payment.totalProviderPayment).to.equal(6780); // $67.80 total
      expect(payment.amountReceivedByPayee).to.equal(5000); // $50.00 to tasker
    });
    ...
    it('should calculate correct fees for $200 service amount', async () => {
      ...
      expect(payment.amount).to.equal(20000);
      expect(payment.applicationFeeAmount).to.equal(2500); // $25.00 platform fee
      expect(payment.providerTaxAmount).to.equal(2925); // $29.25 tax
      expect(payment.totalProviderPayment).to.equal(25425); // $254.25 total
      expect(payment.amountReceivedByPayee).to.equal(20000); // $200.00 to tasker
    });
    ...
    it('should handle withdrawal type correctly (no fees)', async () => {
      ...
      expect(payment.applicationFeeAmount).to.equal(0);
      expect(payment.providerTaxAmount).to.equal(0);
      expect(payment.taxAmount).to.equal(0);
      expect(payment.totalProviderPayment).to.equal(10000);
      expect(payment.amountReceivedByPayee).to.equal(10000);
    });
  });
  ...
  describe('Fee Structure Validation Tests', () => {
    ...
    testCases.forEach(({ serviceAmount, expectedPlatformFee, expectedProviderTax, expectedTotal, expectedTaskerReceives }, index) => {
      it(`should calculate correct fees for test case ${index + 1} ($${(serviceAmount / 100).toFixed(2)})`, async () => {
        ...
        expect(payment.applicationFeeAmount).to.equal(expectedPlatformFee, 
          `Platform fee should be $${(expectedPlatformFee / 100).toFixed(2)}`);
        expect(payment.providerTaxAmount).to.equal(expectedProviderTax, 
        ...
        expect(payment.amountReceivedByPayee).to.equal(expectedTaskerReceives, 
  ...
  describe('Fee Structure Business Rules', () => {
    it('should ensure tasker always receives the full listed amount', async () => {
      ...
      for (const amount of testAmounts) {
        ...
        expect(payment.amountReceivedByPayee).to.equal(amount, 
  ...
  describe('Edge Cases', () => {
    it('should handle minimum service amount correctly', async () => {
      ...
      expect(payment.applicationFeeAmount).to.equal(510); // $5.10
      expect(payment.amountReceivedByPayee).to.equal(100); // Tasker still gets $1.00
    });
    ...
    it('should handle large service amounts correctly', async () => {
      ...
      expect(payment.applicationFeeAmount).to.equal(10500); // $105.00
      expect(payment.amountReceivedByPayee).to.equal(100000); // Tasker gets full $1000
    });
  });
});
