...
export const updateMe = catchAsync(async (req, res, next) => {
  logger.debug(
    `updateMe: User ${req.user.id} attempting to update profile. Body:`,
    req.body
  );
  ...
  const allowedFields = [
    "firstName",
    "lastName", // 'email', // Email changes often need re-verification, handle with care
    "phoneNo",
    "bio",
    "hobbies",
    "skills", // Add skills if in your User model
    "peoplePreference",
    "availability",
    "ratePerHour",
    "address",
  ...
  if (req.body.hobbies !== undefined) {
    ...
    filteredBody.hobbies = hobbiesString
      .split(",")
      .map((h) => h.trim())
      .filter((h) => h);
  }
  ...
  if (req.body.availability && typeof req.body.availability === "object") {
    ...
    logger.warn(
      `updateMe: Invalid availability data received for user ${req.user.id}`
    );
  ...
  if (req.body.address && typeof req.body.address === "object") {
    ...
    logger.warn(
      `updateMe: Invalid address data received for user ${req.user.id}`
    );
  ...
  if (req.file && req.file.fieldname === "profileImage") {
    logger.info(
      `updateMe: Profile image received for user ${req.user.id}. Path: ${req.file.location}`
    );
    ...
    if (user?.profileImageKey && user.profileImageKey !== "default.jpg") {
      logger.info(
        `updateMe: Deleting old profile image from S3: ${user.profileImageKey}`
      );
      const deleteParams = {
      ...
      try {
        ...
        logger.error("updateMe: Failed to delete old profile image from S3", {
          key: user.profileImageKey,
          error: s3DeleteError,
        });
...
export const uploadAlbumPhoto = catchAsync(async (req, res, next) => {
  ...
  if (!req.body.caption || req.body.caption.trim() === "") {
    logger.warn(
      `uploadAlbumPhoto: Missing caption. Deleting S3 object ${req.file.key} for user ${req.user.id}`
    );
    const deleteParams = {
      Bucket: process.env.AWS_S3_BUCKET_NAME,
      Key: req.file.key,
    ...
    try {
      ...
      logger.error(
        "Failed to delete S3 object after captionless album upload",
        { key: req.file.key, error: s3DelErr }
      );
    }
  ...
  if (!user) {
    /* Handle user not found, delete S3 file */ return next(
  ...
  const addedPhoto =
    user.album.id(newPhoto._id) || user.album[user.album.length - 1]; // Get the added subdocument

  logger.info(`Album photo added for user ${req.user.id}`, {
...
export const getUserAlbum = catchAsync(async (req, res, next) => {
  ...
  const userWithAlbum = await User.findById(userIdToFetch).select(
    "album firstName lastName"
  );
  ...
  res.status(200).json({
    ...
    data: {
      album: userWithAlbum.album,
      ownerName: `${userWithAlbum.firstName} ${userWithAlbum.lastName}`.trim(),
    },
  });
});
...
export const deleteAlbumPhoto = catchAsync(async (req, res, next) => {
  ...
  logger.debug(
    `deleteAlbumPhoto: User ${userId} attempting to delete photo ${photoId}`
  );
  ...
  if (photoToDelete.key) {
    const deleteParams = {
      Bucket: process.env.AWS_S3_BUCKET_NAME,
      Key: photoToDelete.key,
    };
    try {
      logger.info(
        `deleteAlbumPhoto: Deleting S3 object ${photoToDelete.key} for user ${userId}`
      );
      await s3Client.send(new DeleteObjectCommand(deleteParams));
      logger.info(`S3 object ${photoToDelete.key} deleted successfully.`);
    } catch (s3DeleteError) {
      logger.error("deleteAlbumPhoto: S3 deletion failed.", {
        key: photoToDelete.key,
        error: s3DeleteError,
      });
    ...
    logger.warn(
      `deleteAlbumPhoto: Photo ${photoId} had no S3 key for user ${userId}. Only removing from DB.`
    );
  }
...
export const matchTaskers = catchAsync(async (req, res, next) => {
  ...
  logger.debug(
    `matchTaskers: Provider ${providerId} searching. Hobbies: [${providerHobbies.join(
      ", "
  ...
  if (providerHobbies.length === 0 && !providerPreference.trim()) {
    ...
    const topTaskers = await User.find({
      ...
      .limit(10)
      .select(
        "firstName lastName fullName profileImage rating ratingCount bio peoplePreference hobbies skills"
      ); // Added skills
  ...
  logger.info(
    `matchTaskers: Found ${matchedTaskers.length} taskers for provider ${providerId}.`
  );

  res.status(200).json({
    status: "success",
    results: matchedTaskers.length,
    data: { taskers: matchedTaskers },
  });
});
