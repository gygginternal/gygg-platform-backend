...
export const getAllGigs = catchAsync(async (req, res, next) => {
  ...
  logger.debug('getAllGigs: Final query object (excluding sort/page/limit/fields):', queryObj);
  ...
  if (req.query.sort) {
    ...
    if (!queryObj.$text || (queryObj.$text && !req.query.sort.includes('score'))) {
  ...
  logger.info(`getAllGigs: Found ${gigs.length} gigs for page ${page} (Total matching: ${totalGigs})`);
...
export const createGig = catchAsync(async (req, res, next) => {
  ...
  logger.info(`createGig: User ${req.user.id} creating gig with title: "${title}"`);

  const newGigData = {
    title, description, category, subcategory, cost, location,
    isRemote, deadline, duration, skills,
...
export const updateGig = catchAsync(async (req, res, next) => {
  ...
  logger.debug(`updateGig: User ${req.user.id} attempting to update gig ID: ${gigId}`);
  ...
  const allowedFields = ['title', 'description', 'category', 'subcategory', 'cost', 'location', 'isRemote', 'deadline', 'duration', 'skills'];
  const updates = {};
  ...
  const updatedGig = await Gig.findByIdAndUpdate(gigId, updates, { new: true, runValidators: true });
...
export const deleteGig = catchAsync(async (req, res, next) => {
  const gigId = req.params.id;
  logger.warn(`deleteGig: User ${req.user.id} attempting to delete gig ID: ${gigId}`);
  ...
  if (!['open', 'cancelled'].includes(gig.status)) {
    return next(new AppError(`Cannot delete gig with status '${gig.status}'. Consider cancelling it first.`, 400));
  }
  ...
  logger.warn(`deleteGig: Gig ${gigId} deleted successfully by user ${req.user.id}.`);
...
export const acceptGig = catchAsync(async (req, res, next) => {
  ...
  if (!gig) { /* ... error ... */ }
  if (gig.status !== 'open') { /* ... error ... */ }
  if (gig.postedBy.id === taskerId) { /* ... error ... */ }
  ...
  try {
      ...
      const newContract = await Contract.create({
          gig: gigId, provider: gig.postedBy.id, tasker: taskerId,
          agreedCost: gig.cost, status: 'pending_payment'
      });
      logger.info(`Contract ${newContract._id} created for Gig ${gigId}`);
      const updatedGigWithPopulatedTasker = await Gig.findById(gigId);
      res.status(200).json({ status: 'success', message: 'Gig accepted. Contract created, awaiting payment.', data: { contractId: newContract._id, gig: updatedGigWithPopulatedTasker } });
  } catch (error) {
      logger.error('--- ACCEPT GIG FAILED (NO TRANSACTION) ---', { /* ... error details ... */ });
      // Revert logic
      const originalGig = await Gig.findById(gigId);
      if (originalGig && originalGig.assignedTo && originalGig.assignedTo.equals(taskerId)) {
          originalGig.assignedTo = null; originalGig.status = 'open';
          try { await originalGig.save(); logger.info(`Gig ${gigId} status successfully reverted.`); }
          catch (revertError) { logger.error(`Failed to revert Gig ${gigId} status.`, { revertError }); }
      }
...
export const matchGigsForTasker = catchAsync(async (req, res, next) => {
  ...
  const taskerPreference = Array.isArray(tasker.peoplePreference) ? tasker.peoplePreference.join(' ') : (tasker.peoplePreference || ''); // Join if array
  const taskerId = tasker._id;

  logger.debug(`matchGigsForTasker: Tasker ${taskerId} searching. Hobbies: [${taskerHobbies.join(', ')}], Pref: "${taskerPreference}"`);

  const pipeline = [];
  pipeline.push({ $match: { status: 'open', postedBy: { $ne: taskerId } } });
  pipeline.push({ $lookup: { from: 'users', localField: 'postedBy', foreignField: '_id', as: 'providerInfo' } });
  pipeline.push({ $unwind: { path: '$providerInfo', preserveNullAndEmptyArrays: true } });
  ...
  if (taskerPreference.trim()) {
      ...
      matchOrConditions.push({
          $or: [
              { 'providerInfo.peoplePreference': { $regex: new RegExp(taskerPreference.trim(), 'i') } }, // If provider pref is string
              { 'providerInfo.peoplePreference': { $in: [new RegExp(taskerPreference.trim(), 'i')] } }, // If provider pref is array
              { 'providerInfo.bio': { $regex: new RegExp(taskerPreference.trim(), 'i') } } // Match bio also
          ]
      });
  }
  ...
  else { logger.info(`matchGigsForTasker: Tasker ${taskerId} has no specific preferences. Showing general open gigs.`); }

  pipeline.push({ $addFields: {
    matchScore: { $add: [
      { $cond: [ { $gt: [ { $size: { $ifNull: [ { $setIntersection: ["$providerInfo.hobbies", taskerHobbies] }, [] ] } }, 0 ] }, 5, 0 ] }, // Hobby match score
  ...
  const page = parseInt(req.query.page, 10) || 1; const limit = parseInt(req.query.limit, 10) || 10; const skip = (page - 1) * limit;
  pipeline.push({ $skip: skip }); pipeline.push({ $limit: limit });
  pipeline.push({ $project: { _id: 1, title: 1, description: 1, category: 1, cost: 1, location: 1, isRemote: 1, createdAt: 1, status: 1, matchScore: 1,
      providerInfo: { _id: "$providerInfo._id", firstName: "$providerInfo.firstName", lastName: "$providerInfo.lastName", fullName: "$providerInfo.fullName", profileImage: "$providerInfo.profileImage", rating: "$providerInfo.rating", peoplePreference: "$providerInfo.peoplePreference", hobbies: "$providerInfo.hobbies" }
  }});

  const gigs = await Gig.aggregate(pipeline);
  logger.info(`matchGigsForTasker: Found ${gigs.length} gigs for tasker ${taskerId}`);
  res.status(200).json({ status: 'success', results: gigs.length, data: { gigs } });
});